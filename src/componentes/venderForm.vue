      ...
      <div class="form-group row">
        ...
        <div class="col-5">
          <input
            ...
            id="cantidad"
            v-model="cantidad"
            v-on:keyup.enter="crearOferta"
            type="number"
            placeholder="W"
          />
        </div>
        <div class="col-4">
          <h5 class="col-form-label" align="left">
            <strong>Total </strong
            ><span class="bg-white px-1"
              >{{ ((cantidad / 1000) * precio).toFixed(2) }}€</span
            >
          </h5>
        </div>
      </div>
      <div class="form-group row">
        <label class="col-3 col-form-label" for="precio"
  ...
  methods: {
      ...
      energy.methods
        ...
        .then((result) => {
          ...
          for (i = 0; i < result.length; i++) {
            ...
            if (
              result[i].seller == this.$store.getters.getAddress &&
              !result[i].done
            ) {
              glob += parseInt(result[i].amount);
            }
		  }
		  //Puede darse que el total utilizado por la batería sea mayor que el total disponible, 
		  // ya que es un random y no todavía está conectado a la batería. Aun así, si se realizase
		  // la venta real, fallaría al no poder enviarse la energía 
    ...
    crearOferta: function () {
      ...
      if (cnt > this.$store.getters.getTotal - this.$store.getters.getUsed) {
        swal(
          "Oops",
          "No puedes poner a la venta más energía de la disponible",
          "error"
        );
        return;
      }
